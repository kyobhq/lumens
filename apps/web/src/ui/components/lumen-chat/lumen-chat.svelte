...
<div
	class="relative overflow-hidden rounded-max-lg bg-lu-main-900 border border-lu-main-750 shadow-border"
>
	<div class="flex justify-between h-full w-md flex-col-reverse overflow-auto relative">
				...
				<div class="flex gap-x-2.5 w-full">
					...
					<div class="flex flex-col flex-1 overflow-hidden">
						<div class="flex gap-x-1 items-baseline">
							...
							<p class="text-xs text-lu-main-500">
								— {timestamp.setLocale('fr').toLocaleString(DateTime.TIME_SIMPLE)}
							</p>
						</div>
						{#if username === 'You'}
		...
		{#if lumen}
			<header
				class="flex flex-col items-center pt-6 sticky top-0 z-1"
				transition:fadeSlideBlur={{ blur: 4, duration: 500, y: 0 }}
			>
				<img
					src={`http://localhost:3333${lumen.avatar}`}
					alt=""
					class="rounded-full aspect-square w-12 bg-lu-accent-100 select-none z-1"
				/>
				<p
					class="text-lu-main-200 rounded-full px-3 py-0.75 -my-2.5 text-xs select-none z-1 liquid-glass"
				>
					{lumen.name ?? 'Lumen'}
				</p>
			</header>
	...
	<div
		role="presentation"
		class="absolute top-0 z-0 h-26 w-full bg-lu-main-900/75 backdrop-blur-md mask-linear-to-black mask-linear-from-transparent pointer-events-none"
	></div>
</div>
